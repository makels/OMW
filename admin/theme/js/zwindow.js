var ZWindow = function(config) {
	...
	this.init = function() {
		...
		$('#zwindow').width(config.width);
		$('#zwindow').height(config.height);
		this.setData();
		this.initHandlers();
	...
	this.setButton = function(config) {
		var id = "";
		if(config.id && config.id) id = "id='" + config.id + "'";
		var button = $("<button " + id + ">" + config.text + "</button>").appendTo('#zwindow .zbuttons');
		if(config.click) {
